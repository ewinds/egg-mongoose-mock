...
module.exports = app => {
  const { options, defaultDB, customPromise, loadModel, plugins } = app.config.mongoose;
  const mongoServer = new MongoMemoryServer();
  const mongoUri = await mongoServer.getUri();

  assert(mongoUri, '[egg-mongoose] url of mock mongo is required');

  // compatibility
  if (!client && !clients && mongoUri) {
